...
import { UploadSimulatorModal } from './UploadSimulatorModal';

export const FreeResourceHub: React.FC = () => {
  const { resources, userUploadedResources, addResource, verifyResource, setActiveTab, navigateToTopic } = useApp();
  ...
  const filteredResources = useMemo(() => {
    const allResources = [...userUploadedResources, ...resources];
  ...
  return (
    <div className="space-y-8 animate-fade-in pb-16">
        ...
        {/* Upload Custom Resource / Generator Button */}
        <div className="bg-gradient-to-br from-slate-900 to-indigo-950 rounded-3xl p-6 md:p-8 text-white shadow-xl flex flex-col md:flex-row items-center justify-between gap-6 border border-slate-800 relative overflow-hidden group">
          <div className="absolute inset-0 bg-[url('https://www.transparenttextures.com/patterns/carbon-fibre.png')] opacity-20 mix-blend-overlay pointer-events-none group-hover:scale-110 transition-transform duration-1000"></div>
          
          <div className="space-y-4 relative z-10">
            <div className="inline-flex items-center space-x-2 bg-indigo-500/20 backdrop-blur-md border border-indigo-500/30 px-3 py-1.5 rounded-full text-xs font-bold text-indigo-300">
              <Sparkles className="w-3.5 h-3.5" />
              <span>Community AI Tool</span>
            </div>
            
            <div>
              <h2 className="text-xl md:text-2xl font-black mb-1">Generate Custom Simulator</h2>
              <p className="text-sm text-slate-400 max-w-xl">
                Have a PDF or a link to a resource? Upload it here to save it to your personal library and automatically generate a custom 300-question practice simulator based on it.
              </p>
            </div>
          </div>
          ...
          <button
            ...
            className="w-full md:w-auto px-6 py-3.5 bg-indigo-600 hover:bg-indigo-500 text-white rounded-2xl font-bold shadow-xl hover:shadow-indigo-500/25 flex items-center justify-center space-x-3 transition-all relative z-10 shrink-0"
          >
      ...
      {showUploadModal && (
        <UploadSimulatorModal onClose={() => setShowUploadModal(false)} />
      )}
